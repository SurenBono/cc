// komodo wallet rig stat + pi-network & TON update
...
const char* crypto = "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,bitget-token,cardano,ethereum,litecoin,pi-network,ripple,solana,the-open-network,verus-coin,worldcoin-wld&vs_currencies=usd";
...
void fetchRig() {
  ...
  {Serial.println("...Rig parse failed!");lcd.clear();("...reparsing rig data");}
    ...
    Serial.println("LuckPoolRig: Komodo Atomic Swap Wallet");
	...
	float Verus = Paid + 1.09357544;  //check wallet current bal
	...
	Serial.println("Komodo Wallet Bal : " + String(Verus,6) + " VRSC =" + " USDT  "+ String(vrscUSD,2));
        Serial.println("====================================================================================");
...
void fetchPrice(){
  ...
  {Serial.println("...parse price failed!");Serial.println();lcd.clear();("...reparsing price");}
    ...
    float bgbPrice = root["bitget-token"]["usd"];
	  ...
	  float piPrice = root["pi-network"]["usd"];
    float tonPrice = root["the-open-network"]["usd"];
    ...
    Serial.print("   SOL " + String(solPrice,2));
    Serial.print("   ADA " + String(adaPrice,2));
    Serial.print("   BGB " + String(bgbPrice,2));
    Serial.println("   TON " + String(tonPrice,2));
    Serial.print("Pi " + String(piPrice,2));
